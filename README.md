# Drupal Testi
The initial repo for Markku's Drupal Testi project

### How do I collaborate on Markku's project?
...
You will call your Drupal Testi project directory something like this (use your own student number!):

```git
    s1900123@FHZV7S2-BC:/home/s1900529/Drupal_Testi/
```

This guide describes how to manage a project using the Linux command line tool bash. 

To start type `bash` into the Windows search (click the magnifying glass icon on the bottom left corner of your screen). As you begin to type the search result will appear above.  Click the bash icon highlighted in blue to open the app. 
...
```git
    s1900123@FHZV7S2-BC:/mnt/c/Windows/System32
```
...
To create a local clone of the Drupal Testi project repository type:

```git
    git clone https://github.com/markku22/Drupal_Testi.git
```
...
```git
    s1900123@FHZV7S2-BC:~/Drupal_Testi$
```

You might not be able to see it, and you don’t necessarily need to see it but there *is* a hidden .git folder in the Drupal Testi directory. This folder contains the git magic that makes everything work. 
...
```git
    ...
    From https://github.com/markku22/Drupal_Testi
...
```git
    https://github.com/markku22/Drupal_Testi
```
